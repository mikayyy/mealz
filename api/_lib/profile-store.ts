...
import type { ProfileInput, Profile, ProfileContext, ProfileResult } from '../../types.js';

export const PROFILE_KEY='default';
const DEFAULT_STORES=["Trader Joe's",'Wegmans'];
function normalizeProfile(input: ProfileInput = {}): Profile{
  const count = (value: unknown): number => Math.max(0,Math.min(20,Math.floor(Number(value)||0)));
  const adults=count(input.adults),children=count(input.children);
  const list = (value: unknown): string[] => Array.isArray(value)?value.filter(x=>typeof x==='string').slice(0,30).map(x=>x.slice(0,100)):[];
...
function requireScope(context: ProfileContext | null | undefined){
  if(!context?.db||!context.householdId)throw new Error('Your account is not linked to a household yet.');
}
export async function readProfile(context: ProfileContext): Promise<ProfileResult>{
...
export async function writeProfile(input: ProfileInput, context: ProfileContext): Promise<ProfileResult>{
  requireScope(context);
  const {db,householdId,userId}=context;
  const p=normalizeProfile(input);
  const row: Record<string, any>={profile_key:PROFILE_KEY,adults:p.adults,children:p.children,household_size:p.householdSize,diet_tags:p.dietTags,equipment:p.equipment,stores:p.stores,updated_at:new Date().toISOString()};
