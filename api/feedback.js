...
export default async function handler(req,res){
  ...
  try{
    ...
    const row=rows[0],tags=Array.isArray(row.tags)?row.tags.filter(t=>t!=='Make again'):[];
    if(makeAgain)tags.push('Make again');
  ...
  }catch(e){console.error('Mealz feedback error',e);return res.status(500).json({error:e.message||'Could not save preference.'})}
}
