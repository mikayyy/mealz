...
export default async function handler(req,res){
  ...
  try{
    ...
    if(existing?.length){const rows=await sb(`meals?select=id&weekly_plan_id=eq.${enc(existing[0].id)}`);if(rows?.length>=6)return res.status(200).json({ok:true,weekStart,prepared:false,reason:'already-ready'})}
    ...
    const recentMeals=await sb('meals?select=title&order=created_at.desc&limit=18');
    const recentTitles=(recentMeals||[]).map(x=>x.title).filter(Boolean).join(', ');
    const prompt=`You are the proactive meal-idea engine for Mealz. Prepare exactly 6 distinct, practical dinner ideas for next week. These are lightweight ideas only: do NOT include recipes, ingredients, quantities, grocery lists, or instructions. Household size: ${householdSize}. Typical cooking days: ${days.join(', ')}. Available equipment: ${equipment.join(', ')}. Household notes from the most recent plan: ${p.notes||'Quick weeknight meals; kid adaptable.'}. Primary store is Trader Joe's; Wegmans is backup. Prefer chicken, turkey, fish, shrimp, tofu, beans, lentils and eggs. Never use beef or pork. Avoid mushrooms when practical. Vary proteins, cuisines and cooking methods. Favor some ingredient overlap without making meals repetitive. Avoid repeating these recent meals when practical: ${recentTitles||'none'}. Return ONLY JSON shaped exactly like {"ideas":[{"id":"unique-slug","title":"Meal title","emoji":"🍽️","description":"one concise sentence","total_minutes":30,"protein":"Turkey","tags":["Kid friendly","Skillet"]}]}. Exactly 6 ideas.`;
    ...
    if(!Array.isArray(data.ideas)||data.ideas.length<6)throw new Error('Did not receive six meal ideas.');
    ...
    const plans=await sb('weekly_plans',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify([{week_start:weekStart,household_size:householdSize,cooking_days:days,equipment,use_up:null,notes:p.notes||null,status:'ideas'}])});
    const plan=plans[0];
    ...
    return res.status(200).json({ok:true,weekStart,prepared:true,count:6});
