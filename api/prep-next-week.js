...
import {sb,enc,supabaseConfigured,ownershipSchemaReady} from './_lib/supabase.js';
...
function ownerClause(ownerId){return ownerId?`&owner_user_id=eq.${enc(ownerId)}`:''}
...
async function prepareOne({ownerId=null,profile=null,profileSource='legacy',weekStart,telemetry}){
  const recentPlans=await sb(`weekly_plans?select=*&status=eq.active${ownerClause(ownerId)}&order=week_start.desc,created_at.desc&limit=12`);
  ...
  const existing=await sb(`weekly_plans?select=id&week_start=eq.${enc(weekStart)}&status=eq.ideas${ownerClause(ownerId)}&limit=1`);
  if(existing?.length){
  ...
  await sb(`weekly_plans?week_start=eq.${enc(weekStart)}&status=eq.ideas${ownerClause(ownerId)}`,{method:'DELETE',headers:{Prefer:'return=minimal'}});
  const planRow={week_start:weekStart,household_size:householdSize,cooking_days:days,equipment,use_up:null,notes:null,status:'ideas'};if(ownerId)planRow.owner_user_id=ownerId;
