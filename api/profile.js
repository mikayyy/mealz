...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='GET'){const result=await readProfile();telemetry.finish(200,{profile:!!result.profile,source:result.source});return res.status(200).json({profile:result.profile,source:result.source})}
    if(req.method==='POST'){
      ...
      telemetry.finish(200,{profile:true,source:result.source,diet_tag_count:result.profile?.dietTags?.length||0,equipment_count:result.profile?.equipment?.length||0});
