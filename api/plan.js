...
async function savePlan(body){
  ...
  try{
    ...
    const oldIds=(priorPlans||[]).map(x=>x.id).filter(id=>id&&id!==plan.id);if(oldIds.length){const list=`(${oldIds.join(',')})`;await sb(`weekly_plans?id=in.${enc(list)}`,{method:'DELETE',headers:{Prefer:'return=minimal'}})}
  ...
  }catch(error){if(newPlanId){try{await sb(`weekly_plans?id=eq.${enc(newPlanId)}`,{method:'DELETE',headers:{Prefer:'return=minimal'}})}catch{}}throw error}
}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='GET'){
      ...
      const data=await readPlan(weekStart);telemetry.finish(200,{week_start:weekStart||data?.plan?.week_start||null,explicit_week:!!weekStart});return res.status(200).json(data||{plan:null,meals:[],groceryItems:[]});
    }
    if(req.method==='POST'){const result=await savePlan(req.body||{});telemetry.finish(200,{week_start:req.body?.weekStart||null,meal_count:req.body?.meals?.length||0,replace_strategy:'create_then_swap'});return res.status(200).json(result)}
    if(req.method==='PATCH'){const result=await updateGrocery(req.body||{});telemetry.finish(200,{operation:'grocery_checked'});return res.status(200).json(result)}
