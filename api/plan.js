...
async function savePlan(body,{db,owned,userId}){
  ...
  try{
    ...
    if(owned&&userId)planRow.owner_user_id=userId;
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='GET'){
      ...
      const data=await readPlan(weekStart,db);telemetry.finish(200,{week_start:weekStart||data?.plan?.week_start||null,explicit_week:!!weekStart,owned});return res.status(200).json(data||{plan:null,meals:[],groceryItems:[]});
    }
    if(req.method==='POST'){const result=await savePlan(req.body||{},{db,owned,userId:auth.id});telemetry.finish(200,{week_start:req.body?.weekStart||null,meal_count:req.body?.meals?.length||0,replace_strategy:'create_then_swap',owned});return res.status(200).json(result)}
    if(req.method==='PATCH'){const result=await updateGrocery(req.body||{},db);telemetry.finish(200,{operation:'grocery_checked',owned});return res.status(200).json(result)}
