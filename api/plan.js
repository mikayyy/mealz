...
async function savePlan(body){
  ...
  if(priorPlans?.length){
    const lists=await Promise.all(priorPlans.map(prior=>sb(`grocery_items?select=name,unit,category,checked&weekly_plan_id=eq.${enc(prior.id)}`)));
    for(const priorGroceries of lists)for(const item of priorGroceries||[])if(item.checked)checkedMap.set(groceryKey(item),true);
  }
  let newPlanId=null;
  try{
    ...
    let groceryPromise=Promise.resolve([]);
    if(groceries.length){groceryPromise=sb('grocery_items',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(groceries)});jobs.push(groceryPromise)}
    await Promise.all(jobs);
    const groceryItems=await groceryPromise;
    const oldIds=(priorPlans||[]).map(x=>x.id).filter(id=>id&&id!==plan.id);
    if(oldIds.length){
      const list=`(${oldIds.join(',')})`;
      await sb(`weekly_plans?id=in.${enc(list)}`,{method:'DELETE',headers:{Prefer:'return=minimal'}});
    }
    ...
    if(newPlanId){
      try{await sb(`weekly_plans?id=eq.${enc(newPlanId)}`,{method:'DELETE',headers:{Prefer:'return=minimal'}})}catch{}
    }
    throw error;
  }
}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='POST'){
      ...
      telemetry.finish(200,{week_start:req.body?.weekStart||null,meal_count:req.body?.meals?.length||0,replace_strategy:'create_then_swap'});
