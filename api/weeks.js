...
export default async function handler(req,res){
  ...
  try{
    ...
    const ids=dedup.map(p=>p.id);let meals=[];if(ids.length){const list=`(${ids.join(',')})`;meals=await sb(`meals?select=id,weekly_plan_id,meal_key,day,title,emoji,total_minutes,sort_order&weekly_plan_id=in.${enc(list)}&order=sort_order.asc`)}
    ...
    telemetry.finish(200,{weeks:summaries.length,past_weeks:past.length});return res.status(200).json({current,next,past});
  }catch(e){
