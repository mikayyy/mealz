...
export default async function handler(req,res){
  ...
  try{
    const currentStart=String(req.query?.current_start||'');
    const nextStart=String(req.query?.next_start||'');
    ...
    const plans=await sb('weekly_plans?select=*&status=eq.active&order=week_start.desc,created_at.desc&limit=24');
    const dedup=[];const seen=new Set();
    for(const p of plans||[]){if(seen.has(p.week_start))continue;seen.add(p.week_start);dedup.push(p)}
    const ids=dedup.map(p=>p.id);let meals=[];
    if(ids.length){const list=`(${ids.join(',')})`;meals=await sb(`meals?select=id,weekly_plan_id,meal_key,day,title,emoji,total_minutes,sort_order&weekly_plan_id=in.${enc(list)}&order=sort_order.asc`)}
    const summaries=dedup.map(p=>summary(p,(meals||[]).filter(m=>m.weekly_plan_id===p.id)));
    const current=summaries.find(w=>w.weekStart===currentStart)||null;
    const next=summaries.find(w=>w.weekStart===nextStart)||null;
    const past=summaries.filter(w=>w.weekStart<currentStart).slice(0,8);
    telemetry.finish(200,{weeks:summaries.length,past_weeks:past.length});
    return res.status(200).json({current,next,past});
  }catch(e){telemetry.fail(e);return res.status(500).json({error:e.message||'Could not load weeks.'})}
}
