...
export default async function handler(req,res){
  ...
  try{
    ...
    const dietary=dietaryInstruction(dietTags),equipmentText=equipmentInstruction(equipment),kid=kidInstruction(children);
    ...
    const data=await callOpenAIJson({prompt,schema:swapSchema,schemaName:'mealz_swap_alternatives',schemaDescription:'Exactly two complete alternative dinner recipes.',timeoutMs:45000,reasoningEffort:'low',maxOutputTokens:5000,telemetry});
    ...
    telemetry.finish(200,{alternative_count:alternatives.length});return res.status(200).json({alternatives});
  }catch(e){
